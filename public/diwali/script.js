...
const store = {
	...
	state: {
		...
		config: {
			quality: String(IS_HIGH_END_DEVICE ? QUALITY_HIGH : QUALITY_NORMAL), // Adaptive quality based on device
			shell: 'Random',
			size: IS_DESKTOP 
				? '4' // Increased desktop size for better visuals
				: IS_HEADER 
					? '1.5' // Slightly larger header shells
					: '2.5', // Larger mobile shells
			autoLaunch: true,
			finale: false,
			skyLighting: SKY_LIGHT_NORMAL + '',
			hideControls: IS_HEADER,
			longExposure: false,
			scaleFactor: IS_DESKTOP ? 1.0 : 0.8 // Optimized scaling
		}
	},
...
function handlePointerStart(event) {
	// Allow the app to temporarily suppress pointer-driven shell launches by
	// setting window.__diwali_ignore_clicks = true. This is used when overlay
	// UI (like a Continue button) needs to accept the click without causing
	// the fireworks to launch.
	try {
		if (window.__diwali_ignore_clicks) {
			return;
		}
	} catch (e) {}
	activePointerCount++;
...
if (IS_HEADER) {
	init();
	// If our wrapper exposed an adjust function, call it to ensure canvases match overlay.
	try {
		const rafAdjust = () => {
			try {
				const fn = (window).__diwali_adjust_canvases;
		...
		rafAdjust();
		setTimeout(rafAdjust, 120);
		setTimeout(rafAdjust, 300);
	} catch (e) {}
} else {
...
// Expose a couple of helpers on the window so external wrappers can control audio
// and trigger sequences (useful when the script is embedded inside another app).
try {
	// `soundManager` is declared with const; expose it explicitly so other scripts can resume audio
	...
	// convenience to resume audio from embedding page
	window.__diwali_resume_audio = function() {
		try {
			if (window.soundManager && typeof window.soundManager.resumeAll === 'function') {
				window.soundManager.resumeAll();
				return true;
			}
			if (window.soundManager && window.soundManager.ctx && typeof window.soundManager.ctx.resume === 'function') {
	...
	// Best-effort stop/cleanup exposed for embedding apps. Calling this will attempt
	// to halt the simulation, remove input/ticker listeners, suspend audio, clear
	// particle queues and canvases, and hide the stage container. This is safe to call
	// multiple times and is intentionally defensive.
	window.__diwali_stop = function() {
		try {
			// Pause simulation state
			if (typeof togglePause === 'function') {
				try { togglePause(true); } catch (e) {}
			} else if (store && store.setState) {
				try { store.setState({ paused: true }); } catch (e) {}
			}
		} catch (e) {}

		// Remove core listeners so no further input or updates are processed
		try { mainStage.removeEventListener('ticker', update); } catch (e) {}
		try {
			mainStage.removeEventListener('pointerstart', handlePointerStart);
			mainStage.removeEventListener('pointerend', handlePointerEnd);
			mainStage.removeEventListener('pointermove', handlePointerMove);
		...
		// Stop audio
		try {
			if (window.soundManager && typeof window.soundManager.pauseAll === 'function') {
				window.soundManager.pauseAll();
			} else if (window.soundManager && window.soundManager.ctx && typeof window.soundManager.ctx.suspend === 'function') {
				window.soundManager.ctx.suspend().catch(()=>{});
			}
		} catch (e) {}

		// Prevent auto-launch sequences
		try { autoLaunchTime = Infinity; } catch (e) {}

		// Clear particle queues (best-effort)
		try {
			COLOR_CODES.forEach(color => {
				try {
					if (Star && Star.active && Star.active[color]) Star.active[color].splice(0, Star.active[color].length);
					if (Spark && Spark.active && Spark.active[color]) Spark.active[color].splice(0, Spark.active[color].length);
				} catch (e) {}
			});
			if (BurstFlash && BurstFlash.active) BurstFlash.active.splice(0, BurstFlash.active.length);
		} catch (e) {}

		// Clear canvases
		try {
			if (trailsStage && trailsStage.ctx) trailsStage.ctx.clearRect(0, 0, trailsStage.width || 0, trailsStage.height || 0);
			if (mainStage && mainStage.ctx) mainStage.ctx.clearRect(0, 0, mainStage.width || 0, mainStage.height || 0);
		} catch (e) {}

		// Hide the stage container so embedding UI can transition away
		try { document.querySelector('.stage-container') && document.querySelector('.stage-container').classList.add('remove'); } catch (e) {}
	};
} catch (e) {
	// ignore
}